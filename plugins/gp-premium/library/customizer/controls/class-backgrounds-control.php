...
if ( class_exists( 'WP_Customize_Control' ) ) {
	class GeneratePress_Background_Images_Customize_Control extends WP_Customize_Control {
		...
		public function to_json() {
			...
			$this->json[ 'position_title' ] = esc_html__( 'left top, x% y%, xpos ypos (px)', 'backgrounds' );
			$this->json[ 'position_placeholder' ] = esc_html__( 'Position', 'backgrounds' );
		...
		public function get_repeat_choices() {
			return array(
				'' => esc_html__( 'Repeat','backgrounds' ),
				'repeat-x' => esc_html__( 'Repeat x','backgrounds' ),
				'repeat-y' => esc_html__( 'Repeat y','backgrounds' ),
				'no-repeat' => esc_html__( 'No Repeat','backgrounds' )
			);
		}

		public function get_size_choices() {
			return array(
				'' => esc_html__( 'Size (Auto)','backgrounds' ),
				'100' => esc_html__( '100% Width','backgrounds' ),
				'cover' => esc_html__( 'Cover','backgrounds' ),
				'contain' => esc_html__( 'Contain','backgrounds' )
			);
		}

		public function get_attachment_choices() {
			return array(
				'' => esc_html__( 'Attachment','backgrounds' ),
				'fixed' => esc_html__( 'Fixed','backgrounds' ),
				'local' => esc_html__( 'Local','backgrounds' ),
				'inherit' => esc_html__( 'Inherit','backgrounds' )
			);
		}
	}
}
