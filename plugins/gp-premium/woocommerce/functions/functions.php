...
// Import/Export
require plugin_dir_path( __FILE__ ) . 'import-export.php';
...
function generatepress_wc_scripts() {
	...
	wp_enqueue_script( 'generate-woocommerce', plugin_dir_url( __FILE__ ) . "js/woocommerce{$suffix}.js", array( 'jquery' ), GENERATE_WOOCOMMERCE_VERSION );
...
function generatepress_wc_product_columns( $columns ) {
...
function generatepress_wc_cart_link() {
	...
	ob_start();
	?>
	<a href="<?php echo esc_url( wc_get_cart_url() ); ?>" class="cart-contents" title="<?php esc_attr_e( 'View your shopping cart','generate-woocommerce' ); ?>"><?php echo $icon; ?><span class="amount"><?php if ( WC()->cart->subtotal > 0 ) { echo wp_kses_data( WC()->cart->get_cart_subtotal() ); } ?></span></a>
	<?php
	return ob_get_clean();
}
...
function generatepress_wc_rating_html($rating_html, $rating) {
	if ( $rating > 0 ) {
		$title = sprintf( __( 'Rated %s out of 5', 'generate-woocommerce' ), $rating );
	} else {
	...
	$rating_html .= '<span style="width:' . ( ( $rating / 5 ) * 100 ) . '%"><strong class="rating">' . $rating . '</strong> ' . __( 'out of 5', 'generate-woocommerce' ) . '</span>';
	$rating_html .= '</div>';
	return $rating_html;
}
