...
function generate_do_select_page_header_meta_box( $post ) {
    ...
    <?php else : ?>
		<p>
			<?php
			printf( __( 'No Page Headers found. Want to <a href="%1$s" target="_blank">create one</a>?', 'page-header' ),
...
function show_generate_page_header_meta_box( $post ) {
	...
	$content_required = sprintf(
		...
		__( 'Content is required for the below settings to work.', 'page-header' ),
	...
	<div id="generate-tabs-container">
		<ul class="generate-tabs-menu">
			<li class="generate-current image-settings">
				<a href="#generate-image-tab"><?php _e( 'Image', 'page-header' ); ?></a>
			</li>

			<li class="content-settings">
				<a href="#generate-content-tab"><?php _e( 'Content', 'page-header' ); ?></a>
			</li>

			<li class="video-settings">
				<a href="#generate-video-background-tab"><?php _e( 'Background Video', 'page-header' ); ?></a>
			</li>

			<?php if ( generate_page_header_logo_exists() || generate_page_header_navigation_logo_exists() ) : ?>
				<li class="logo-settings">
					<a href="#generate-logo-tab"><?php _e( 'Logo','page-header' ); ?></a>
				</li>
			<?php endif; ?>

			<li class="advanced-settings">
				<a href="#generate-advanced-tab"><?php _e( 'Advanced', 'page-header' ); ?></a>
			</li>
		...
		<div class="generate-tab">
				...
				<div class="page-header-image-settings" <?php echo $show_image_settings; ?>>
					...
					<p>
						...
						<select name="_meta-generate-page-header-enable-image-crop" id="_meta-generate-page-header-enable-image-crop">
							<option value="" <?php selected( generate_page_header_get_post_meta( get_the_ID(), '_meta-generate-page-header-enable-image-crop', true ), '' ); ?>><?php _e( 'Disable', 'page-header' );?></option>
							<option value="enable" <?php selected( generate_page_header_get_post_meta( get_the_ID(), '_meta-generate-page-header-enable-image-crop', true ), 'enable' ); ?>><?php _e( 'Enable', 'page-header' );?></option>
						</select>
					</p>
				...
				<div style="margin-top:12px;">
					...
					<div class="page-header-column">
						<p>
							...
							<label for="_meta-generate-page-header-content-autop"><?php _e( 'Automatically add paragraphs', 'page-header' );?></label>
						</p>
						...
						<p>
							...
							<label for="_meta-generate-page-header-content-padding"><?php _e( 'Add padding', 'page-header' );?></label>
						</p>
						...
						<p>
							...
							<label for="_meta-generate-page-header-image-background"><?php _e( 'Add background image', 'page-header' );?></label>
						</p>
						...
						<p class="parallax">
							...
							<label for="_meta-generate-page-header-image-background-overlay"><?php _e( 'Use background color as overlay', 'page-header' );?></label>
						</p>
						...
						<p class="parallax">
							...
							<label for="_meta-generate-page-header-image-background-fixed"><?php _e( 'Parallax effect', 'page-header' );?></label>
						</p>
						...
						<p class="fullscreen">
							...
							<label for="_meta-generate-page-header-full-screen"><?php _e( 'Fullscreen', 'page-header' );?></label>
						</p>
						...
						<p class="vertical-center">
							...
							<label for="_meta-generate-page-header-vertical-center"><?php _e( 'Vertical center content', 'page-header' );?></label>
						</p>
					</div>

					<div class="page-header-column">
						<p>
							<label for="_meta-generate-page-header-image-background-type" class="example-row-title"><strong><?php _e( 'Container', 'page-header' );?></strong></label><br />
							<select name="_meta-generate-page-header-image-background-type" id="_meta-generate-page-header-image-background-type">
								<option value="" <?php selected( generate_page_header_get_post_meta( get_the_ID(), '_meta-generate-page-header-image-background-type', true ), '' ); ?>><?php _e( 'Contained', 'page-header' );?></option>
								<option value="fluid" <?php selected( generate_page_header_get_post_meta( get_the_ID(), '_meta-generate-page-header-image-background-type', true ), 'fluid' ); ?>><?php _e( 'Full width', 'page-header' );?></option>
							</select>
						</p>

						<p>
							<label for="_meta-generate-page-header-image-background-type" class="example-row-title"><strong><?php _e( 'Inner container', 'page-header' );?></strong></label><br />
							<select name="_meta-generate-page-header-inner-container" id="_meta-generate-page-header-inner-container">
								<option value="" <?php selected( generate_page_header_get_post_meta( get_the_ID(), '_meta-generate-page-header-inner-container', true ), '' ); ?>><?php _e( 'Contained', 'page-header' );?></option>
								<option value="full" <?php selected( generate_page_header_get_post_meta( get_the_ID(), '_meta-generate-page-header-inner-container', true ), 'full' ); ?>><?php _e( 'Full width', 'page-header' );?></option>
							</select>
						</p>
						...
						<p>
							...
							<select name="_meta-generate-page-header-image-background-alignment" id="_meta-generate-page-header-image-background-alignment">
								<option value="" <?php selected( generate_page_header_get_post_meta( get_the_ID(), '_meta-generate-page-header-image-background-alignment', true ), '' ); ?>><?php _e( 'Left', 'page-header' );?></option>
								<option value="center" <?php selected( generate_page_header_get_post_meta( get_the_ID(), '_meta-generate-page-header-image-background-alignment', true ), 'center' ); ?>><?php _e( 'Center', 'page-header' );?></option>
								<option value="right" <?php selected( generate_page_header_get_post_meta( get_the_ID(), '_meta-generate-page-header-image-background-alignment', true ), 'right' ); ?>><?php _e( 'Right', 'page-header' );?></option>
							</select>
						</p>
						...
						<p>
							<label for="_meta-generate-page-header-left-right-padding" class="example-row-title"><strong><?php _e( 'Left & right padding', 'page-header' );?></strong></label><br />
				...
				<div id="generate-logo-tab" class="generate-tab-content">
					<?php if ( function_exists( 'generate_get_defaults' ) ) {
						...
						if ( function_exists( 'generate_construct_logo' ) && ( '' !== $generate_settings[ 'logo' ] || get_theme_mod( 'custom_logo' ) ) ) {
							...
							<p class="description" style="margin-top:0;">
								<?php _e( 'Overwrite your site-wide logo/header on this page.','page-header' ); ?>
							</p>
					...
					if ( function_exists( 'generate_menu_plus_get_defaults' ) ) {
						...
						if ( '' !== $generate_menu_plus_settings[ 'sticky_menu_logo' ] ) {
							?>
							<p class="description" style="margin-top:0;">
								<?php _e( 'Overwrite your navigation logo on this page.', 'page-header' ); ?>
							</p>
			...
			<div id="generate-advanced-tab" class="generate-tab-content" style="display:none">
				...
				<p style="margin-top:0;">
					...
					<label for="_meta-generate-page-header-combine"><?php _e( 'Merge with site header', 'page-header' );?></label>
				</p>
				...
				<div class="combination-options">
					<p class="absolute-position">
						...
						<label for="_meta-generate-page-header-absolute-position"><?php _e( 'Place content behind header (sliders etc..)', 'page-header' );?></label>
					</p>

					<p>
						<label for="_meta-generate-page-header-site-title" class="example-row-title"><?php _e( 'Site title', 'page-header' );?></label><br />
					...
					<p>
						<label for="_meta-generate-page-header-site-tagline" class="example-row-title"><?php _e( 'Site tagline', 'page-header' );?></label><br />
					...
					<p>
						...
						<label for="_meta-generate-page-header-transparent-navigation"><?php _e( 'Custom navigation colors', 'page-header' );?></label>
					</p>

					<div class="navigation-colors">
						<p>
							<label for="_meta-generate-page-header-navigation-background" class="example-row-title"><strong><?php _e( 'Navigation background', 'page-header' );?></strong></label><br />
...
function generate_page_header_tags_add_meta_box( $post ) {
	add_meta_box( 'generate_page_header_tags', __( 'Template Tags', 'page-header' ), 'generate_page_header_tags_do_meta_box', 'generate_page_header', 'side', 'low' );
}
...
function generate_page_header_tags_do_meta_box( $post ) {
	...
	<p class="decription" style="margin-top:0;opacity:0.8;font-size:85%;"><?php _e( 'The content title of the current post/taxonomy.', 'page-header' ); ?></p>

	<input type="text" readonly="readonly" value="{{post_date}}" />
	<p class="decription" style="margin-top:0;opacity:0.8;font-size:85%;"><?php _e( 'The published date of the current post.', 'page-header' ); ?></p>

	<input type="text" readonly="readonly" value="{{post_author}}" />
	<p class="decription" style="margin-top:0;opacity:0.8;font-size:85%;"><?php _e( 'The author of the current post.', 'page-header' ); ?></p>

	<input type="text" readonly="readonly" value="{{post_terms.taxonomy}}" />
	<p class="decription" style="margin-top:0;opacity:0.8;font-size:85%;"><?php _e( 'The terms attached to the chosen taxonomy (category, post_tag, product_cat).', 'page-header' ); ?></p>

	<input type="text" readonly="readonly" value='{{custom_field.name}}' />
	<p class="decription" style="margin-top:0;opacity:0.8;font-size:85%;"><?php _e( 'Custom post meta. Replace "name" with the name of your custom field.', 'page-header' ); ?></p>
	<?php
}
...
function generate_premium_page_header_menu_item() {
	?>
	<li class="page-heade-meta-menu-item"><a href="#generate-layout-page-header"><?php _e( 'Page Header', 'page-header' ); ?></a></li>
	<?php
}
