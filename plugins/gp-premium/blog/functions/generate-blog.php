...
if ( ! function_exists( 'generate_blog_scripts' ) ) {
	...
	function generate_blog_scripts() {
		...
		if ( ( 'true' == generate_blog_get_masonry() && generate_blog_get_columns() ) || ( $settings[ 'infinite_scroll' ] && ! is_singular() ) ) {
...
if ( ! function_exists( 'generate_blog_load_more' ) ) {
	...
	function generate_blog_load_more() {
		...
		if ( ( ! $settings[ 'infinite_scroll_button' ] || ! $settings[ 'infinite_scroll' ] ) || is_singular() ) {
			return;
		}
...
add_action( 'after_setup_theme', 'generate_blog_disable_infinite_scroll_srcset' );
/**
